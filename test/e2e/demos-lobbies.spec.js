...
test.describe('/demos/lobbies', () => {
	test('renders every control and enforces id/code constraints without joining', async ({ page }) => {
		...
		await expect(page.getByTestId('lob-code-input')).toHaveAttribute('maxlength', '8')
		await expect(page.getByTestId('lob-random')).toHaveText('random')
		await expect(page.getByTestId('lob-create')).toHaveText('Join')
		...
		await expect(page.getByTestId('lob-error')).toHaveText('Table ids are numeric (1-9 digits).')
		...
		await expect(page.getByTestId('lob-error')).toHaveText('That code does not decode.', { timeout: 10_000 })
