...
<script>
	...
	const DEMOS = [
		...
		{ slug: 'cluster-cron',      title: 'Cluster cron: one leader, one tick',             desc: 'Redis-backed leader election visualised. <code>createLeader</code> + <code>configureCron({ leader })</code> gate a 1Hz <code>live.cron</code> so it fires once cluster-wide instead of once per worker.' },
		...
		{ slug: 'effect',            title: 'live.effect: one publish, three streams',     desc: 'An <code>orders</code> stream\'s publishes trigger a <code>live.effect([\'orders\'], handler)</code> that fans out to an audit feed and a notifications feed. Three streams in one page; the effect handler is fire-and-forget.' },
