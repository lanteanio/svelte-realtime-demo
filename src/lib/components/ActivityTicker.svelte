<!--
	ActivityTicker - fixed bar at the bottom showing recent board activity.

	Shows the 5 most recent actions (note created, edited, deleted, etc).
	Items are ephemeral - they come from the activity live stream, not
	from the database. If you refresh the page, the ticker starts empty.

	Each entry has a colored dot (matching the user's identity color),
	the user's name, and a short description of what they did.
-->
<script>
	let { items = [] } = $props()
	// Show the 5 newest entries, most recent first
	const latest = $derived(items.slice(-5).toReversed())
</script>

<div class="fixed bottom-0 left-0 right-0 bg-base-200/80 backdrop-blur-sm
						border-t border-base-300 px-4 py-1.5 flex gap-6 overflow-hidden text-xs">
	<!--
		Composite key (ts + user + action) because two CRUD events can land
		in the same millisecond on a busy board, and Date.now() alone is
		not a unique identifier. The collision is rare but triggers Svelte's
		each_key_duplicate runtime error when it does.
	-->
	{#each latest as item (item.ts + '|' + item.user + '|' + item.action)}
		<span class="whitespace-nowrap flex items-center gap-1">
			<span class="w-2 h-2 rounded-full inline-block" style:background={item.color}></span>
			<span class="font-semibold">{item.user}</span>
			{item.action}
		</span>
	{/each}
	{#if latest.length === 0}
		<span class="opacity-40">No activity yet - double-click the canvas to add a note</span>
	{/if}
</div>
